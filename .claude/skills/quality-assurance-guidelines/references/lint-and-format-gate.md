...
- MUST flag a new ESLint's inline suppression (with justification) directive that lacks an inline justification on the same line. The project rule is "explain why, not just what".
- MUST flag a new block-level ESLint suppression in any file that is not one of the whitelisted exception points the linter config defines.
