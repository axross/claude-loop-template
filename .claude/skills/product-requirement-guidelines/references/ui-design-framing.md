...
A UI design section describes hierarchy, layout intent, and content priority in spec terms — what's primary, what's secondary, what's optional — not specific components, class names, or markup. That separation is what keeps this reference from duplicating the project's own UI/component skill (created during INIT): the spec states _what_ the surface must convey, the implementation skill owns _how_ it's built.
