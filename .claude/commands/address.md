...
- MUST resolve human decisions through the question UI inline when it returns an answer (see [Asking the Human](#asking-the-human)); end the turn for `/address continue` when a synchronous answer can't be obtained or a machine event is stuck, and never schedule a wake-up to re-check for human input.
